...
//查询会议列表
func (ac *AppletController) GetReservations(c *gin.Context) {
	//获取并绑定请求参数
...
//查询会议详情
func (ac *AppletController) GetReservation(c *gin.Context) {
	//请求参数处理
...
//新增会议预约
func (ac *AppletController) CreateReservation(c *gin.Context) {
	//获取并绑定请求参数
...
//查询会议室列表
func (ac *AppletController) GetRooms(c *gin.Context) {
	//获取并绑定请求参数
	...
	res, err := serviceclient.RoomServiceClient.GetRooms(context.TODO(), &proto.ReqGetRooms{
		...
		OneStatus: &proto.ReqGetRooms_Status{Status: "1"},
		Page:      params.Page,
		PageSize:  params.PageSize,
		Order:     params.Order,
		SortBy:    params.SortBy,
...
//查询会议室详情
func (ac *AppletController) GetRoom(c *gin.Context) {
	//请求参数处理
	id, _ := strconv.ParseInt(c.Param("id"), 10, 64)
	if id <= 0 {
		c.JSON(http.StatusOK, lib.NewResponse(1001).WithMsg("id不能为空"))
...
//查询地点列表
func (ac *AppletController) GetSpaces(c *gin.Context) {
	//获取并绑定请求参数
	...
	res, err := serviceclient.SpaceServiceClient.GetSpaces(context.TODO(), &proto.ReqGetSpaces{
		...
		Order:     params.Order,
		SortBy:    params.SortBy,
		OneStatus: &proto.ReqGetSpaces_Status{Status: "1"},
	})
	if err != nil {
		log.Info("未知错误：", err)
		c.JSON(http.StatusOK, lib.NewResponse(1000))
